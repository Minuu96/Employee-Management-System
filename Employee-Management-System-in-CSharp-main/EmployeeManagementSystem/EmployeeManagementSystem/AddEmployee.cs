...
namespace EmployeeManagementSystem
{
    ...
    {
        SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;Initial Catalog=employee;Integrated Security=True;Connect Timeout=30");
                        ...
                        using(SqlCommand checkEm = new SqlCommand(checkEmID, connect))
                        {
                            ...
                            else
                            {
                                ...
                                string insertData = "INSERT INTO employees " +
                                    ...
                                    ", position, image, salary, date_added, status) " +
                                    "VALUES(@employeeID, @fullName, @gender, @contactNum" +
                                    ", @position, @image, @salary, @insertDate, @status)";

                                string directoryFolder = Path.Combine(Application.StartupPath, "Directory");
                                string path = Path.Combine(directoryFolder, addEmployee_id.Text.Trim() + ".jpg");
